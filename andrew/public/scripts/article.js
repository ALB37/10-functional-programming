...
(function (module){
  ...
  /* REVIEWED: In Lab 8, we explored a lot of new functionality going on here. Let's re-examine the concept of context.
  ...
  Article.numWordsAll = () => {
    return Article.all.map(article => article.body.split(' ').reduce((acc => acc + 1), 0)).reduce((acc, num) => acc += num);
  };
  ...
  Article.allAuthors = () => {
    return Article.all.map(article => article.author).reduce((acc, author) => {
      if (acc.indexOf(author) === -1 ){
        acc.push(author);
      }
      return acc;
    }, []);
  ...
  Article.truncateTable = callback => {
    $.ajax({
      ...
      .then(console.log)
    // REVIEWED: Check out this clean syntax for just passing 'assumed' data into a named function!
